...
argument-hint: <cache-dir> <app-name> [--force]
---
...
especializados em paralelo sobre o cache → `consolidated.json` + `gaps.md` +
`ANALYSIS.md` em `<cache-dir>/apps/<app-name>/analysis/` (workspace isolado da
run). Se `app-name` faltar: use o único workspace existente em
`<cache-dir>/apps/`, ou pergunte se houver vários.
...
- Pré-requisitos: `crawl.json` complete + `ds-spec.md` aprovado (senão, aponte a
  fase faltante).
- Se `<workspace>/analysis/consolidated.json` já existe e não há `--force`,
  apenas reporte o existente.
...
- Ao final, sugira `/ds-build <cache-dir> <app-name> <app-dir>`.
