...
argument-hint: <cache-dir> <app-name> [--ref=<ds-app-dir>] [--baseline]
---
...
→ `<cache-dir>/apps/<app-name>/ds-spec.md` aprovado (workspace isolado da run —
outros workspaces do mesmo site são invisíveis).

Siga o procedimento completo em `${CLAUDE_PLUGIN_ROOT}/references/pipeline/brainstorm.md`.

Resumo do contrato:
- `cache-dir` aponta para uma extração existente (`~/.ds-cache/<slug>` com
  `crawl.json`); `app-name` identifica a run (sem ele: derive do pedido ou pergunte).
